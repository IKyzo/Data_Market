    ...
    if response.status_code == 200:
        ...
        if job_elements:
            ...
            for job in job_elements:
                ...
                job_title_cleaned = job_title_element.text.replace('\n', '')
                job_description_element = job.find('p')
                
                # Extracting company name and location
                # Find all matching elements and choose the second one (index 1)
                company_info_elements = job.find_all('a', href=lambda value: value and value.startswith('/offres-emploi/companies/'))
                company_info_element = company_info_elements[1] if len(company_info_elements) > 1 else None
                
                company_name_element = company_info_element.find('b').get_text(strip=True) if company_info_element and company_info_element.find('b') else company_info_element.get_text(strip=True) if company_info_element else "Company not found"

                # Extracting job location
                job_location_element_html = str(job.find('i', class_='fa fa-map-marker'))
                location_element = job.find('i', class_='fa fa-map-marker').next_sibling
                job_location_element = location_element.strip() if location_element else None

                print("Job Title:", job_title_cleaned if job_title_cleaned else "Title not found")
                print("Description:", job_description_element.text if job_description_element else "Description not found")
                print("Company Name Element:", company_info_element)
                if company_info_element:
                    company_name_bold = company_info_element.find('b')
                    if company_name_bold:
                        print("Company Name Text (Bold):", company_name_bold.get_text(strip=True))
                    else:
                        print("Company Name Text:", company_info_element.get_text(strip=True))

                print("Job Location Element HTML:", job_location_element_html)
                print("Job Location Text:", str(job_location_element))
                ...
                job_titles.append(job_title_cleaned.strip() if job_title_cleaned else "Title not found")
                job_descriptions.append(job_description_element.text.strip() if job_description_element else "Description not found")
                company_names.append(company_name_element if company_name_element else "Company not found")
                job_locations.append(job_location_element if job_location_element else "Location not found")
                tags.append(tag)
